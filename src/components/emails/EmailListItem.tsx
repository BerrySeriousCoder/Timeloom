...
export const EmailListItem: React.FC<EmailListItemProps> = ({
  ...
  return (
    <div className={`flex items-center hover:bg-muted cursor-pointer ${!email.read ? 'bg-blue-50 dark:bg-gray-800 font-semibold' : ''} ${selectedEmails.includes(email.id) ? 'bg-blue-100 dark:bg-gray-700' : ''}`}>
      <div className="p-3 flex items-center w-full">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button onClick={(e) => onToggleStar(email.id, e)} className="p-0 m-0 bg-transparent border-none">
                  <Star
                    className={`h-4 w-4 transition-colors ${email.starred ? 'text-amber-400 fill-amber-400' : 'text-gray-400 hover:text-gray-500'}`}
                  />
                </button>
              </TooltipTrigger>
        ...
        <div className="flex-grow min-w-0 mr-4">
          <div className="flex justify-between items-baseline">
            <span className={`text-sm truncate max-w-[150px] sm:max-w-[220px] md:max-w-[300px] ${!email.read ? 'font-bold' : ''}`}>
              {email.sender}
            </span>
          </div>
          <h3 className={`text-sm truncate ${!email.read ? '' : 'text-gray-700'}`}>
            {email.subject}
          </h3>
              ...
              {availableTags.map(tag => {
                ...
                return (
                  <DropdownMenuItem key={tag.id} className="flex justify-between items-center p-0">
                    <label htmlFor={`list-tag-${email.id}-${tag.id}`} className="flex items-center justify-between w-full px-2 py-1.5 cursor-pointer">
                      <span>{tag.name} ({tag.type})</span>
                      <Checkbox
                        id={`list-tag-${email.id}-${tag.id}`}
                        checked={isChecked}
                        onCheckedChange={(checked) => onTagToggle(email.id, tag.id, { stopPropagation: () => {} } as React.MouseEvent)}
                        className="ml-2"
